...
void *sbrk(size_t incr) {
  ...
  static uint8_t *p = (uint8_t *) &__heap_start;
  static uint8_t *newp;

  newp = p + incr;
  if ((int) newp > ((int) &__heap_start + (int) &__heap_size)) {
    /*errno = ENOMEM;
    return (void *)-1;*/
  }
  return p = newp;
...
#endif
{
  ...
  INTERNAL_SIZE_T    sbrk_size     = nb + DEFAULT_TOP_PAD + MINSIZE;
  ...
  (void) correction_failed; // avoid "unused variable warning"
  assert(((unsigned long)((char*)top + top_size) & (pagesz - 1)) == 0 || correction_failed);
}
...
#else
  //RARG Void_t* mem;
  //asm volatile("add %0, t2, zero" : "=r"(mem));
  //printf(" fREe:%x ",mem);
